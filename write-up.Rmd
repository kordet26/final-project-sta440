---
title: "Candidate Quality and Demographic Trends in the 2022 United States House of Representatives elections"
author: "Kevin Ordet"
...
df %>%
  ...
  theme_bw() +
  labs(y= "Dem performance over expectation (POE)", x="Partisan lean",
       fill="Incumbent", color="Incumbent",title="Dem performance over expectation (POE) vs. Partisan lean")
```

The plot above shows the Democratic performance over expectation`poe` of each district plotted against the partisan lean, with the points again colored by incumbent party `inc`.
Democrats overperformed in districts plotted above the dashed line and underperformed in
districts plotted below the dashed line. 
...
to beat partisan lean in open seats with a lean between R+5 and R+15 (i.e. "Lean R" seats). Further analysis of the qualities of these districts and candidates will help explain this phenomenon.
...
df %>%
  ...
  arrange(desc(poe)) %>%
  kable(digits=1, caption="Performance over Expectation (POE) in open Lean R seats (PVI: R+15 to R+5)")
...
outran expectations in 9 of the 12 districts and by an average of 2.1 points. It should
be noted that all six Republican election deniers contesting these seats ran behind partisan lean; the only full acceptor ran almost 10 points ahead of partisanship.
...
df %>%
  ...
  theme_bw()  +
  labs(y= "Dem performance over expectation (POE)", x="Dem fundraising share (%)",
       fill="Incumbent", color="Incumbent",title="Dem performance over expectation (POE) vs. Dem fundraising share (%), by Incumbent party")
...
df %>%
  ...
  geom_smooth(alpha=0.1, method="lm") +
  theme_bw()  +
  labs(y= "Dem performance over expectation (POE)", x="College educated population (%)",
       fill="Incumbent", color="Incumbent",title="Dem performance over expectation (POE) vs. College educated population (%), by Incumbent party")
...
df %>%
  ...
  geom_smooth(alpha=0.1, method="lm") +
  theme_bw()  +
  labs(y= "Dem performance over expectation (POE)", x="Urbanization index",
       fill="Incumbent", color="Incumbent",title="Dem performance over expectation (POE) vs. Urbanization index, by Incumbent party")
```


The plots above show the Democratic outperformance `poe` plotted against
...
while Republicans did so in urban and low college-educated seats. Second, increased fundraising tended to result in overperformance of partisanship,and incumbents dominated fundraising.
...
df %>%
  ggplot(mapping=aes(x=gop_stance, y=poe, fill=gop_stance))+ facet_wrap(~ pvi_group2) +    geom_hline(yintercept=0) +
    geom_boxplot()+
  theme_bw() +
  labs(y= "Dem performance over expectation (POE)", x="Republican stance on 2020",
       fill="Republican stance on 2020", color="Republican stance on 2020",title="Dem performance over expectation (POE) vs. Republican stance on 2020, by PVI group")
...
for this occurrence; strong Republican voters may have been more likely to turn out
for election deniers in uncompetitive races they otherwise would not have. Regardless,
the critical takeaway is the relative success of election-accepting Republicans in
competitive districts, which ultimately decide the outcome of the election.
...
df %>%
  ...
  labs(y= "Average Dem performance over expectation (POE)", x="Average partisan lean",
       title="State-by-state average performance over expectation (POE) vs. average partisan lean")
```
The plot above shows the average Democratic overperformance `poe` by state plotted
againt the state's average partisan lean `pvi_22`. The labels are sized by the number of
